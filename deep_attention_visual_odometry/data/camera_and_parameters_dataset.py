...
class CameraAndParametersDataset(Dataset[CameraViewsAndPoints]):
    ...
    def __init__(self, epoch_length: int, num_points: int, num_views: int, camera_model):
        self.epoch_length = int(epoch_length)
        self._num_points = int(num_points)
        self._num_views = int(num_views)
        self._camera_parameter_means = torch.tensor([640., 320., 240.], dtype=torch.float32)
        self._camera_parameter_std = torch.tensor([100., 100., 100.], dtype=torch.float32)
        self._camera_distance_mean = 15.
        self._camera_distance_std = 5.0
        self._camera_location_spread = torch.tensor([3., 3., 3.], dtype=torch.float32)
        self._camera_target_spread = torch.tensor([3., 3., 3.], dtype=torch.float32)
        self._camera_up_spread = torch.tensor([3., 3., 3.], dtype=torch.float32)
        self._transform_std = torch.tensor([5., 5., 5., torch.pi / 3, torch.pi / 3, torch.pi / 3], dtype=torch.float32)
        self._points_std = torch.tensor([3., 3., 3.], dtype=torch.float32).reshape(1, 3, 1)
    ...
    def __getitem__(self, item: int) -> CameraViewsAndPoints:
        ...
        projected_points, projection_weights = self._project_points(world_points, camera_extrinsics, camera_intrinsics)
    ...
    def _generate_world_points(self) -> torch.Tensor:
        world_points = self._points_std * torch.randn(self._num_points - 1, 3, dtype=torch.float32)
    ...
    def _generate_camera_extrinisics(self, world_centre: torch.Tensor) -> torch.Tensor:
        ...
        camera_direction = camera_direction / torch.linalg.vector_norm(camera_direction, keep_dim=True)
        up_direction = torch.randn(3, dtype=torch.float32)
        up_direction = up_direction / torch.linalg.vector_norm(up_direction, keep_dim=True)
        camera_distance = (self._camera_distance_mean + self._camera_distance_std * torch.randn(1, dtype=torch.float32)).abs()
        view_distance = (self._camera_distance_mean + self._camera_distance_std * torch.randn(1, dtype=torch.float32)).abs()
        up_distance = (self._camera_distance_mean + self._camera_distance_std * torch.randn(1, dtype=torch.float32)).abs()
        ...
        camera_locations = camera_location_base[None, :] + self._camera_location_spread[None, :] * torch.randn(self._num_views, 3, dtype=torch.float32)
        camera_targets = camera_target_base[None, :] + self._camera_target_spread[None, :] * torch.randn(self._num_views, 3, dtype=torch.float32)
        camera_up = up_base[None, :] + self._camera_up_spread[None, :] * torch.rand(self._num_views, 3, dtype=torch.float32)
    ...
    def _generate_camera_intrinsics(self) -> torch.Tensor:
        camera_parameters = self._camera_parameter_means + torch.randn(3) * self._camera_parameter_std
        return torch.tensor([[
            [camera_parameters[0], 0., camera_parameters[1], 0.],
            [0., camera_parameters[0], camera_parameters[2], 0.],
            [0., 0., 1., 0.]
        ]])

    def _project_points(self, world_points: torch.Tensor, camera_extrinisics, camera_intrinisics: torch.Tensor, ) -> tuple[torch.Tensor, torch.Tensor]:
        augmented_points = torch.cat([world_points, torch.ones_like(world_points[:, 0:1])])
    ...
    def _generate_transform(self) -> torch.Tensor:
        ...
        rot_z = torch.tensor([
            [cos_angles[1], -sin_angles[1], 0.],
            [sin_angles[1], cos_angles[1], 0.],
            [0., 0, 1.],
        ])
